...
cell = for file in readdir(FOLDERNAME)
    ...
    eps_g_low = (1 .- quantiles[:εₑ⁻].nt[:var"2.5%"][1]).*(1 .- quantiles[:frac_sol_am_neg].nt[:var"2.5%"][1])
    eps_g_mid = (1 .- quantiles[:εₑ⁻].nt[:var"50.0%"][1]).*(1 .- quantiles[:frac_sol_am_neg].nt[:var"50.0%"][1])
    eps_g_high = (1 .- quantiles[:εₑ⁻].nt[:var"97.5%"][1]).*(1 .- quantiles[:frac_sol_am_neg].nt[:var"97.5%"][1])
    ...
    push!(cells, cell)
    push!(n_li_lower_bound, eps_g_low)
    push!(n_li_upper_bound, eps_g_mid)
    push!(n_li_50, eps_g_high)
    #push!(mle_vec, mle)
end

df = DataFrame(cells=cells, lower_bound=n_li_lower_bound, upper_bound=n_li_upper_bound, mid=n_li_50)
