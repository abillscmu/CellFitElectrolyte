...
function fit_cfe_degradation(cycle_array_vec, data_dict, cells, p_thermal, cyc)
    #(this would be rand for SG)
    err = 0.0
    num = 0
    for cell in cells
        cell_integer = parse(Int, split(cell, "VAH")[2])
        cycle_array = cycle_array_vec[cell_integer][cyc:cyc]

        ω = mean(data_dict[cell]["distributions"][cyc][:ω].data)
        εₑ⁻ = mean(data_dict[cell]["distributions"][cyc][:εₑ⁻].data)
        εₑ⁺ = mean(data_dict[cell]["distributions"][cyc][:εₑ⁺].data)
        frac_sol_am_neg = mean(data_dict[cell]["distributions"][cyc][:frac_sol_am_neg].data)
        frac_sol_am_pos = mean(data_dict[cell]["distributions"][cyc][:frac_sol_am_pos].data)
...
function loss(p_thermal::T) where {T <: ComponentArray}
    try
        mse=0
        for cyc in [500]
        #THERMALS
            df = CSV.read("/home/abills/Data/cycle_individual_data/VAH01_$cyc.csv", DataFrame)
            ...
            sol = fit_cfe_degradation(cycle_array_vec, data_dict,cells, p_thermal, cyc)
            return sol
        end
        return mse
    catch
...
function loss(p_thermal)
    T_amb, C, ha, hb = p_thermal
    p_deg = ComponentArray(
        T_amb = T_amb,
        C = C,
        ha = ha,
...
[1.0, 48.52559886527583, 0.005659969188357159, 0.011298387604847037]

#CellFitElectrolyte.anneal(loss, p_thermal, ub, lb)
