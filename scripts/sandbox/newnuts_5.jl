...
@model function fit_cfe(interpolated_voltage)
    ...
    frac_sol_am_pos ~ truncated(Normal(1, 0.1),0.475, 1.0)
    frac_sol_am_neg ~ truncated(Normal(1, 0.1),0.475, 1.0)
...
θ = [0.02, 0.25, 0.25, 0.9, 0.9]



# Sample 3 independent chains with forward-mode automatic differentiation (the default).
chain = sample(model, NUTS(0.65), MCMCSerial(), 10, 1; progress=true)
d = Dict("chain" => chain)

save("results/newnuts_5/$(VAH)_HMC.jld2", d)
sleep(5)
