...
function evaluator(p::ComponentVector{T}) where {T}
    ...
    func = ODEFunction((du::Array{T,1},u::Array{T,1},p::ComponentVector{T},t::T)->CellFitElectrolyte.equations_electrolyte_concentrationdependent(du,u,p,t,cache,cellgeometry,cathodeocv,anodeocv))
    ...
    for step::Int in 1:num_steps-1
        ...
        endV[step] = CellFitElectrolyte.calc_voltage_concentrationdependent(integrator.u,integrator.p,integrator.t,cache,cellgeometry,cathodeocv,anodeocv,values[step])
        endT[step] = Temp
    end
...
function evaluator_ca(p::ComponentVector{T}) where {T}
