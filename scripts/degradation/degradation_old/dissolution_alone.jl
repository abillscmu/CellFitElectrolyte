...
function lifetime_evaluator(p::ComponentVector{T}, cycle_array_vec, cycles_to_save) where {T}
    ...
    function f(du, u, p, t)
        ...
        @unpack R⁺ = p_phys
        @unpack εₑ⁻ = p_phys
        @unpack Vₛ⁺, T⁺ = cellgeometry
        #get stuff for dissolution model out of state vector
        cₛˢ⁺ = u[7]
        cₑ⁺ = u[6]
        Iapp = u[8]
        a⁺ = 3*εₛ⁺/R⁺
        ...
        J₀⁺ = CellFitElectrolyte.exchange_current_density(cₛˢ⁺,cₑ⁺,cathodeocv.c_s_max,p_phys.k₀⁺)
        J⁺ = Iapp./A⁺
        η⁺ = CellFitElectrolyte.butler_volmer(J₀⁺, J⁺, Temp)
        @unpack U_diss, i0_diss, α_diss = p_deg
        η_diss = (U⁺ - η⁺) - U_diss 
        i_diss = CellFitElectrolyte.active_material_dissolution(i0_diss, a⁺, α_diss, Temp, η_diss)
        du[9] = i_diss
        q = cathodeocv.c_s_max*T⁺
