...
@model function fit_cfe_degradation(cycle_array_vec, data_dict, cells)
    k_sei_neg ~ Normal(1e-15, 1e-20)
    D_sei ~ Normal(1.2e-22, 1e-21)
    c_s_bulk_neg ~ Normal(1000, 100)
    Vₘ ~ Normal(1.3e-5, 1e-6)
    U_sei ~ Normal(3.5, 0.2)
    ...
    for cell in cells
        ...
        for (i, cycle) in enumerate(sorted_cycles)
            ...
            probability = KDEDistributions.pdf(data_dict["VAH01"]["distributions"][cycle][:εᵧ⁻], u_out[10, i])
            if probability >= 0
                Turing.@addlogprob! KDEDistributions.logpdf(data_dict["VAH01"]["distributions"][cycle][:εᵧ⁻], u_out[10,i])
            else
                @warn "negative probability encountered..."
                Turing.@addlogprob! -Inf*probability
            end
        end
    end
    return nothing
...
mymodel = fit_cfe_degradation(cycle_array_vec, data_dict, cells)


chain = sample(mymodel, Emcee(2), 10)


@save "sei_VAH01_emcee.jld2" chain