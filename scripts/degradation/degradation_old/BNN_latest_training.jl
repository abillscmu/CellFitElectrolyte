...
@model function fit_cfe_degradation(cycle_array_vec, data_dict, cells)
    #only trying to estimate NN parameters for now...
    ...
    for cell in cells
        ...
        for (i, cycle) in enumerate(sorted_cycles)
            for (j, sym) in enumerate(predicted_states)
                var = u_out[i][8+j]
                log_probability = Distributions.logpdf(data_dict[cell]["distributions"][cycle][sym], var)
                Turing.@addlogprob! log_probability
            end
        end
    end
    return nothing
...
model = fit_cfe_degradation(cycle_array_vec, data_dict, cells)

chain = sample(model, MH(), 2)

@save "test_var.jld2" chain