...
ax[0].set_yticks((1:num_mechanisms).-0.5, deg_df.Name)
ax[0].get_xaxis().set_visible(false)
ax[1].pcolormesh(heatmap_matrix[:,:,2],vmin=-1,vmax=1,cmap=cmap)
ax[1].set_yticks((1:num_mechanisms).-0.5, deg_df.Name)
ax[1].get_xaxis().set_visible(false)
ax[2].pcolormesh(heatmap_matrix[:,:,3],vmin=-1,vmax=1,cmap=cmap)
ax[2].set_yticks((1:num_mechanisms).-0.5, deg_df.Name)
ax[2].set_xticks((1:length(cells)).-0.5,cells,rotation=45, ha="right",rotation_mode="anchor")
