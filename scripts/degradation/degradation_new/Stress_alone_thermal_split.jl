...
function lifetime_evaluator(p::ComponentVector{T}, cycle_array_vec, cycles_to_save, f_0) where {T}
...
function degradation_simulator(cell, p_deg)
    ...
    for (i, cycle) in enumerate(sorted_cycles)
        ...
        err += (data_dict[cell]["mean"][cycle][:frac_sol_am_pos] - u_out[10, i]).^2
    end
    return err
end
...
#@save "stress_$(cell)_fullycoupled.jld2" sol