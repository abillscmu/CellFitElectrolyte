...
@everywhere function lifetime_evaluator(p::ComponentVector{T}, cycle_array_vec, cycles_to_save) where {T}
    ...
    u[11] = p.ics.εₑ⁻
    u[12] = p.ics.εₑ⁺
    u[13] = p.ics.frac_sol_am_neg
    u[14] = p.ics.frac_sol_am_pos
    
    #length > 15 indicates an augmented state.
    if length(u) >= 15
        u[15:end] .= 0
    end
    ...
    function f(du, u, p, t)
        @unpack p_deg, p_phys, p_thermal = p
        
        ω = u[10]
        εₑ⁻ = u[11]
        εₑ⁺ = u[12]
        frac_sol_am_neg = u[13]
        frac_sol_am_pos = u[14]

        p_phys[:εₛ⁻] = (1 - εₑ⁻)*frac_sol_am_neg
        p_phys[:εₛ⁺] = (1 - εₑ⁺)*frac_sol_am_pos
        ...
        T_normalized = (Temp - 273)/70
        ω = (u[10] - 0.01)/0.04
        εₑ⁻ = (u[11] - 0.05)/0.45
        εₑ⁺ = (u[12] - 0.05)/0.45
        frac_sol_am_neg = (u[13] - 0.5)/0.5
        frac_sol_am_pos = (u[14] - 0.5)/0.5
        if input_dim <= 14
            u_in = SVector(xₛˢ⁻, xₛᵇ⁻, xₑ⁻, xₑˢ, xₑ⁺, xₛᵇ⁺, xₛˢ⁺,Iapp,T_normalized, ω, εₑ⁻, εₑ⁺, frac_sol_am_neg, frac_sol_am_pos)
        else
            u_in = SVector(xₛˢ⁻, xₛᵇ⁻, xₑ⁻, xₑˢ, xₑ⁺, xₛᵇ⁺, xₛˢ⁺,Iapp,T_normalized, ω, εₑ⁻, εₑ⁺,frac_sol_am_neg, frac_sol_am_pos, u[15:end]...)
        end
        new_u = nn(u_in, p_deg)
        du[10:end] .= new_u
        return nothing
...
@everywhere function degradation_simulator(cell, p_deg)
    ...
    for (i, cycle) in enumerate(sorted_cycles)
        ...
        for (j, sym) in enumerate(predicted_states)
            err += (data_dict[cell]["mean"][cycle][sym] - u_out[9+j, i]).^2
        end
    end
    return err
end
...
p_deg = zeros(length(Float64.(initial_params(nn))./(1*10^divisor)))
