...
d = load("results/outputs0117_elec/VAH01_51_HMC.jld2")
chain = d["chain"]
...
other_cycle_array = CellFitElectrolyte.current_profile(interpolated_current,interpolated_time)
...
function chain_to_expected(chain,idx)
    return (ω = mean(chain[:ω].data[idx,1]), εₑ⁺=mean(chain[:εₑ⁺].data[idx,1]),εₑ⁻ = mean(chain[:εₑ⁻].data[idx,1]), frac_sol_am_pos = mean(chain[:frac_sol_am_pos].data[idx,1]),frac_sol_am_neg=mean(chain[:frac_sol_am_neg].data[idx,1]))
end
...
rmse = zeros(N)
i=1
c = chain_to_expected(chain,i)
predicted = fit_cfe(interpolated_voltage,c)

figure(1)
clf()
plot(interpolated_time[1:end-1],predicted)
