...
function lifetime_evaluator(p, cycle_array_vec, u, k_resistance, saves, k_sei, E_sei, k_pore)
    #Create Function (with neural network)
    println((k_sei, E_sei, k_resistance, k_pore))
    function f(du, u, p, t)
...
@model function turing_life_fit(distribution_dict, cycle_array_vec, lifetime_evaluator, fitting_cycles)
    ...
    for vah in keys(fitting_cycles)
        ...
        sol = try 
            sol = lifetime_evaluator(p, cycle_array_vec[cellnum][first_cycle:last_cycle], u, k_resistance, saves, k_sei, E_sei, k_pore)
        catch
            @info "Solver errored"
            sol = 0
        end
...
function run_thru(distribution_dict, cycle_array_vec, lifetime_evaluator, fitting_cycles, k_sei_exponent, E_sei, k_resistance_exponent, k_pore_exponent)
...
k_pore_exponent = opt.values[4]

my_sol = run_thru(distribution_dict, cycle_array_vec, lifetime_evaluator, fitting_cycles, k_sei_exponent, E_sei, k_resistance_exponent, k_pore_exponent)

cell_to_plot = "VAH01"

@save "lifetime_results/kinetic_resistance_pore_noplate_$(dataset).jld2" my_sol distribution_dict fitting_cycles opt
