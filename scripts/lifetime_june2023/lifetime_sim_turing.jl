...
p_phys = ComponentVector{Any}(θₛ⁻ = 6.130391775012598e-10, θₑ = 3e-6, θₛ⁺ = 3.728671559985511e-8, R⁺ = 4.2902932816468984e-6, R⁻ = 6.7447548850488327e-6, β⁻ = 1.5, β⁺ = 1.5, βˢ = 1.5, εₛ⁻ = εₛ⁻, εₛ⁺ = εₛ⁺, εᵧ⁺ = εᵧ⁺, εᵧ⁻ = εᵧ⁻, c = 50.0, h = 0.1, Tamb = 298.15, Temp = 298.15, k₀⁺ = 1e-1, k₀⁻ = 1e-1, x⁻₀ = 0.6, εₑˢ = 0.8, cₑ₀ = 2000.0, κ = 0.25, t⁺ = 0.38, input_type = 3.0, input_value = 4.2, ω = ω, Eₑ = 50.0, Eₛ⁺ = 50.0, Eₛ⁻ = 50.0, cccv_switch=false, cccv_switch_2=false, vfull=vfull, ifull=-0.01)
...
@model function turing_life_fit(distribution_dict, cycle_array_vec, p, lifetime_evaluator)
    ...
    ω_end = sol[9]
    ω_end ~ distribution_dict[:ω]["Final"]
    return nothing
end
...
p_nn = zeros(10)
T = eltype(p_nn)
p = ComponentArray{Any}(p_phys = p_phys, p_nn = p_nn)
...
model = turing_life_fit(distribution_dict, cycle_array_vec, p, lifetime_evaluator)



chain = sample(model, NUTS(0.65), MCMCSerial(), 5, 1; progress=true)
